...
function Dashboard({ subscriptions, lowStock, go }: { subscriptions: Subscription[]; lowStock: Product[]; go: (v: View) => void }) {
  ...
  const scheduled = subscriptions.filter((s) => s.status !== "Cancelada");
  const today = scheduled.filter((s) => s.next.startsWith("Hoy")).length;
  return <div className="space-y-8"><section><SectionHeading title="Resumen del negocio" text="Datos actualizados de la operación diaria" /><div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4"><StatCard icon={Users} label="Suscripciones activas" value={String(subscriptions.filter((s) => s.status === "Activa").length)} note={`${subscriptions.length} clientes en este local`} /><StatCard icon={CircleDollarSign} label="Ingreso mensual estimado" value={money(revenue)} note="Proyección recurrente" tone="accent" /><StatCard icon={AlertTriangle} label="Alertas de stock" value={String(lowStock.length)} note="Requieren reposición" tone="warning" /><StatCard icon={PackageCheck} label="Próximas entregas" value={String(scheduled.length)} note={`${today} hoy · ${scheduled.length - today} próximas`} /></div></section>
